...
def convert_ros_pc2_to_o3d(msg):
    ...
    for i in range(msg.width * msg.height):
        ...
        try:
            if x_dtype == 7 and y_dtype == 7 and z_dtype == 7: # FLOAT32
                x = np.frombuffer(msg.data[start_byte + x_offset:start_byte + x_offset + 4], dtype=np.float32)[0]
                y = np.frombuffer(msg.data[start_byte + y_offset:start_byte + y_offset + 4], dtype=np.float32)[0]
                z = np.frombuffer(msg.data[start_byte + z_offset:start_byte + z_offset + 4], dtype=np.float32)[0]
...
def get_closest_timestamp(timestamp, timestamps_dict):
    ...
    if not timestamps_dict: return None
...
def ransac_coarse_alignment(source, target, source_fpfh, target_fpfh, voxel_size, ransac_threshold=5):
    ...
    result = o3d.pipelines.registration.registration_ransac_based_on_feature_matching(
        ...
        criteria=o3d.pipelines.registration.RANSACConvergenceCriteria(max_iteration=4000, confidence=0.999),
    )
    if result.fitness > 0.1: return result.transformation
    return None

def detect_loop_closure(
    current_frame_idx, current_pcd_processed, current_fpfh,
    accumulated_pcds, accumulated_fpfhs, accumulated_transforms,
    voxel_size, search_radius=10.0, loop_closure_fitness_thresh=0.3, temporal_window=100
):
    """Detects loop closure by finding similar frames in the accumulated map."""
    loop_closures = []
    if current_frame_idx < temporal_window: return loop_closures

    current_pos = accumulated_transforms[current_frame_idx][:3, 3]

    searchable_indices = list(range(current_frame_idx - temporal_window))
    if not searchable_indices: return loop_closures
    
    historical_positions = np.array([accumulated_transforms[i][:3, 3] for i in searchable_indices])
    if len(historical_positions) == 0: return loop_closures
    ...
    if not candidate_indices: return loop_closures

    for candidate_idx in candidate_indices:
        try:
            ## FIX 3 (Performance): Check if the candidate has features before using it.
            ...
            coarse_transform = ransac_coarse_alignment(
                current_pcd_processed, accumulated_pcds[candidate_idx],
                current_fpfh, candidate_fpfh, voxel_size
            )
            if coarse_transform is None: continue

            reg_p2l = o3d.pipelines.registration.registration_icp(
                current_pcd_processed, accumulated_pcds[candidate_idx],
                voxel_size * 2, coarse_transform,
                o3d.pipelines.registration.TransformationEstimationPointToPlane(),
                o3d.pipelines.registration.ICPConvergenceCriteria(max_iteration=20),
            )

            if reg_p2l.fitness > loop_closure_fitness_thresh:
                loop_closures.append((candidate_idx, reg_p2l.transformation, reg_p2l.fitness))
        except Exception:
            continue
...
def process_bag(args):
    ...
    if args.odom_topic: print(f"Odometry topic: {args.odom_topic}")
    if args.imu_topic: print(f"IMU topic: {args.imu_topic}")
    if args.enable_loop_closure: print(f"Loop closure detection: ENABLED (Search every {args.loop_closure_search_interval} frames)")
    else: print("Loop closure detection: disabled")
    ...
    if args.odom_topic: topics_to_read.append(args.odom_topic)
    if args.imu_topic: topics_to_read.append(args.imu_topic)

    with AnyReader([bag_path], default_typestore=typestore) as reader:
        connections = [x for x in reader.connections if x.topic in topics_to_read]
        if not connections: sys.exit(f"Error: No messages found for topics: {topics_to_read}")
        
        print("\nExtracting messages from bag file...")
        for conn, timestamp, rawdata in tqdm(reader.messages(connections=connections), desc="Reading Messages"):
            try:
                msg = reader.deserialize(rawdata, conn.msgtype)
                if conn.topic == args.pc_topic:
                    pcd = convert_ros_pc2_to_o3d(msg)
                    if pcd and len(pcd.points) > 100: point_clouds.append((timestamp, pcd))
                elif conn.topic == args.odom_topic:
                    transform = get_odom_transform(msg)
                    if transform is not None: odom_data[timestamp] = transform
                elif conn.topic == args.imu_topic:
                    try:
                        quat = msg.orientation
                        imu_data[timestamp] = R.from_quat([quat.x, quat.y, quat.z, quat.w]).as_matrix()
                    except (AttributeError, ValueError): continue
            except Exception: continue

    if not point_clouds: sys.exit("Error: No valid point clouds were extracted.")
    print(f"\nExtracted {len(point_clouds)} point clouds")
    if args.odom_topic: print(f"Extracted {len(odom_data)} odometry messages")
    if args.imu_topic: print(f"Extracted {len(imu_data)} IMU messages")

    # --- 2. Pairwise Registration and Pose Graph Construction ---
    pose_graph = o3d.pipelines.registration.PoseGraph()
    current_transform = np.identity(4, dtype=np.float64)
    pose_graph.nodes.append(o3d.pipelines.registration.PoseGraphNode(current_transform))

    source_pcd_raw = point_clouds[0][1]
    source_pcd_processed = source_pcd_raw.voxel_down_sample(args.voxel_size)
    source_pcd_processed.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=args.voxel_size * 2, max_nn=30))

    source_fpfh = None
    if args.enable_loop_closure:
        ## FIX 3 (Performance): Compute FPFH for first frame if it's a keyframe.
    ...
    if args.odom_topic:
        ...
        if closest_ts: previous_odom_transform = odom_data[closest_ts]

    ## FIX 2 (Fragile Indexing): Use a simple counter instead of complex logic.
    node_id_counter = 1 
    successful_node_indices = [0] # Keep track of original cloud indices that were added.
    ...
    for i in pbar:
        ...
        target_pcd_processed.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=args.voxel_size * 2, max_nn=30))
        ...
        if args.odom_topic:
            ...
            if closest_ts:
                ...
                if previous_odom_transform is not None:
                    initial_guess = np.linalg.inv(previous_odom_transform) @ current_odom_transform
                previous_odom_transform = current_odom_transform

        try:
            reg_p2l = o3d.pipelines.registration.registration_icp(
                source_pcd_processed, target_pcd_processed, args.icp_dist_thresh,
                initial_guess, o3d.pipelines.registration.TransformationEstimationPointToPlane(),
        ...
        if reg_p2l.fitness > args.icp_fitness_thresh:
            ...
            ## FIX 2 (Fragile Indexing): Use the simple counter to add the consecutive edge.
            pose_graph.edges.append(
                o3d.pipelines.registration.PoseGraphEdge(
                    node_id_counter - 1, node_id_counter, reg_p2l.transformation,
            ...
            target_fpfh = None
            ## FIX 3 (Performance): Only compute FPFH for keyframes.
            if args.enable_loop_closure and (i % args.loop_closure_search_interval == 0):
                target_fpfh = compute_fpfh_descriptor(target_pcd_processed, args.voxel_size)
            accumulated_fpfhs.append(target_fpfh)

            if args.enable_loop_closure and (i % args.loop_closure_search_interval == 0):
                pbar.set_description("Registering (LC Search)")
                loop_closures = detect_loop_closure(
                    node_id_counter, target_pcd_processed, target_fpfh,
                    accumulated_pcds, accumulated_fpfhs, accumulated_transforms,
                    args.voxel_size, search_radius=args.loop_closure_radius,
                    loop_closure_fitness_thresh=args.loop_closure_fitness_thresh,
                    temporal_window=100,
                )

                for lc_idx, lc_transform, lc_fitness in loop_closures:
                    ## FIX 1 (Logic Bug): Information matrix must be proportional to fitness.
                    # Higher fitness = higher confidence = larger information value.
                    lc_information = np.eye(6) * lc_fitness * 100.0

                    pose_graph.edges.append(
                        o3d.pipelines.registration.PoseGraphEdge(
                            lc_idx, node_id_counter, lc_transform,
                            lc_information, uncertain=True
                        )
                    )
            ...
            ## FIX 2 (Fragile Indexing): Increment counter and log original index on success.
            node_id_counter += 1
    ...
    if args.enable_loop_closure: print(f"\nLoop closures detected: {loop_closures_found}")

    # --- 3. Global Optimization ---
    print("\nOptimizing the pose graph...")
    option = o3d.pipelines.registration.GlobalOptimizationOption(
        max_correspondence_distance=args.icp_dist_thresh, edge_prune_threshold=0.25, reference_node=0
    )
    try:
    ...
    except Exception as e: print(f"Warning: Global optimization failed: {e}")

    # --- 4. Final Map Generation and Meshing ---
    print("\nGenerating final point cloud and mesh...")
    pcd_combined = o3d.geometry.PointCloud()

    ## FIX 2 (Fragile Indexing): Use the robust list of successful indices to merge clouds.
    for node_idx, original_pcd_idx in tqdm(enumerate(successful_node_indices), desc="Merging Clouds"):
    ...
    if len(pcd_combined.points) == 0: sys.exit("Error: Combined point cloud is empty.")

    if args.level_floor:
        print("Attempting to level the floor...")
        try:
            plane_model, inliers = pcd_combined.segment_plane(distance_threshold=args.voxel_size * 2, ransac_n=3, num_iterations=1000)
            a, b, c, d = plane_model
            normal = np.array([a, b, c]) / np.linalg.norm(np.array([a, b, c]))
            target_normal = np.array([0, 0, 1.0])
            if np.dot(normal, target_normal) < 0: normal = -normal
    ...
    pcd_combined_downsampled = pcd_combined.voxel_down_sample(voxel_size=args.voxel_size)
    ...
    try:
        mesh, densities = o3d.geometry.TriangleMesh.create_from_point_cloud_poisson(pcd_combined_downsampled, depth=9)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Convert ROS 2 Bag files with PointCloud2 data to a 3D mesh.")
    ...
    parser.add_argument("--odom_topic", default=None, help="Odometry topic for initial guess.")
    parser.add_argument("--imu_topic", default=None, help="IMU topic for orientation.")
    ...
    parser.add_argument("--icp_fitness_thresh", type=float, default=0.6, help="Minimum ICP fitness score (0-1).")
